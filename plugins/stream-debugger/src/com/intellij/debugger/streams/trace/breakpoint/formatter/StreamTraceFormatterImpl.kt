...
class StreamTraceFormatterImpl(private val valueManager: ValueManager) : StreamTraceFormatter {
  ...
  override fun formatTraceResult(chain: StreamChain, collectedValues: StreamTraceValues): ArrayReference {
    val intermediateStepsValues = collectedValues.intermediateOperationValues
    return valueManager.watch {
      val peekFormatter = PeekTraceFormatter(valueManager)
      val terminatorFormatter = TerminatorTraceFormatter(valueManager, collectedValues)
